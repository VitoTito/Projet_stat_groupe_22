...
significant_p_values <- matrix(FALSE, nrow = nrow(p_values), ncol = ncol(p_values))

# Appliquer la condition pour ne garder que les valeurs < 0.05
p_values[is.na(p_values)] <- ''
p_values[p_values >= 0.05] <- ''
p_values[is.nan(p_values)] <- ''
p_values <- ifelse(is.na(p_values), '', p_values)
p_values
p_values[p_values == ''] <- 10
p_values <- apply(p_values, c(1, 2), as.numeric)
p_values <- round(p_values, digits = 4)
p_values <- ifelse(p_values == 10, '', p_values)

# Exporter p_values dans un fichier CSV
write.csv(p_values_contingency, file = "C:/Users/Vito/Desktop/D�p�t Projet Statistique 2A/Projet_stat_groupe_22/p_values.csv", row.names = FALSE)
...
# D�finir le seuil de signification
seuil_sign2 <- 0.05
...
p_values_contingency[is.na(p_values_contingency)] <- ''
p_values_contingency[p_values_contingency >= 0.05] <- ''
p_values_contingency[is.nan(p_values_contingency)] <- ''
p_values_contingency <- ifelse(is.na(p_values_contingency), '', p_values_contingency)
p_values_contingency
p_values_contingency[p_values_contingency == ''] <- 10
p_values_contingency <- apply(p_values_contingency, c(1, 2), as.numeric)
p_values_contingency <- round(p_values_contingency, digits = 4)
p_values_contingency <- ifelse(p_values_contingency == 10, '', p_values_contingency)

# Exporter p_values_contingency dans un fichier CSV
write.csv(p_values_contingency, file = "C:/Users/Vito/Desktop/D�p�t Projet Statistique 2A/Projet_stat_groupe_22/p_values_contingency.csv", row.names = FALSE)

# Afficher la matrice de p-values des tests du chi-deux pour les variables cat�gorielles
